...
function renderPages(){const el=$("#page-list");if(!el)return;el.innerHTML=adminPages.map((p,i)=>'<article class="page-admin-card"><div><strong>'+esc(p[0])+'</strong><small>'+esc(p[1])+'</small></div><div class="page-actions"><a class="btn btn-outline" href="'+p[1]+'" target="_blank" rel="noopener">View Page</a><button class="btn btn-primary edit-page" type="button" data-page="'+i+'">Edit Page</button></div></article>').join("");$(".edit-page").forEach(b=>b.onclick=()=>openPageEditor(Number(b.dataset.page)))}

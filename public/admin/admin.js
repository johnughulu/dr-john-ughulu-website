...
function renderPages(){const el=$("#page-list");if(!el)return;el.innerHTML=adminPages.map(p=>'<article class="page-admin-card"><div><strong>'+esc(p[0])+'</strong><small>'+esc(p[1])+'</small></div><a class="btn btn-outline" href="'+p[1]+'" target="_blank" rel="noopener">View Page</a></article>').join("")}
